    ...
    try {
        ...
        if (Object.keys(body).length > 0) {
            ...
            if (req.files.product_image) {
                ...
                returnedPicture = await cloudinary.uploader.upload(
                    pictureToUpload,
                    {
            ...
            res.status(400).json({
                error: {
                    message:
                        "These fields should be completed: picture, title, price, size",
                },
            });
        }
    } catch (error) {
    ...
    try {
        if (query.id) {
            ...
            if (offerSearched) {
                ...
                res.status(400).json({
                    error: { message: "The offer was not found" },
                });
            }
        } else {
            res.status(400).json({
                error: { message: "An id has to be specified in the query" },
            });
        }
    } catch (error) {
...
Router.get("/offers", async (req, res) => {
    ...
    try {
        ...
        for (const key in q) {
            if (key === "title")
                search["product_name"] = new RegExp(q[key], "i"); // Search by name
            if (key === "priceMin") {
    ...
    try {
        if (params.id) {
            ...
            if (offerSearched) {
                ...
                res.status(400).json({
                    error: { message: "No offer was found" },
                });
            }
        }
    } catch (error) {
